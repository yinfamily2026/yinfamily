// 尹氏家谱 - 世系树渲染（SVG，支持缩放/拖动/点击查看）
(function () {
  const COL_W = 132, ROW_H = 96, NODE_W = 112, NODE_H = 46;
  ...
  function layout(members) {
    ...
    childrenOf.forEach(arr => arr.sort((a, b) => (a.generation - b.generation) || a.name.localeCompare(b.name, 'zh')));
  ...
  function render(svg, data, onSelect) {
    ...
    for (const { m, x, y } of centers) {
      ...
      t1.setAttribute('x', x); t1.setAttribute('y', ry + (spouse ? 19 : 27));
      ...
      if (spouse) {
        ...
        t2.setAttribute('x', x); t2.setAttribute('y', ry + 37);
